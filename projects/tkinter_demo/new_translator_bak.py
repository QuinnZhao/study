...
class Application(Tk):
    ...
    def _split_to_blocks(self):
        ...
        if len(contents) < WORDS_NUM_LIMIT:
            return [contents]
        contents += '\n' * (WORDS_NUM_LIMIT - len(contents) % WORDS_NUM_LIMIT)
        blocks = [contents[i: i + WORDS_NUM_LIMIT] for i in range(0, len(contents), WORDS_NUM_LIMIT)]
